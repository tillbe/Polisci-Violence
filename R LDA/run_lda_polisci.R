...
c = mongo(db = "politics", collection = "debates")
df = c$find('{"debate": 1}')


# data is stored in variable `text`
text = df$text # insert data here in form of vector...
  
# remove all texts with less than 100 characters.
...
runLDA <- function(k, SEED = 2015){
  ...
  pbPost("note", paste0("Model with ", k," topics done!"), "Done!")
  return(l)
}
